  ...
  persist(
    (set, get) => ({
      ...
      toggleSeat: (seat) => {
        ...
        const sAny = schedule as any;
        const destLower = (sAny?.route?.destination || sAny?.destination || '').toLowerCase();
        const fallbackFare = schedule?.fare || (destLower.includes('chittagong') ? 1200 : destLower.includes('cox') ? 2000 : 2000);
